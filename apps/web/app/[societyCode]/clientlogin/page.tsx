...
export default function ClientLoginPage() {
  ...
  const copy = getSiteCopy(locale).clientPortal;
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.location.href = getDefaultDashboardPath("CLIENT", response.user.requiresPasswordChange, response.user.allowedModuleSlugs);
    } catch (caught) {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 4, md: 8 } }}>
      <Card className="surface-vibrant fade-rise" sx={{ overflow: "hidden", borderRadius: 4 }}>
        <Grid container>
          <Grid size={{ xs: 12, md: 5 }} sx={{ p: { xs: 2.5, md: 4.5 }, color: "#fff", position: "relative", overflow: "hidden", zIndex: 1, background: "linear-gradient(145deg, #1e3a8a 0%, #1d4ed8 100%)", "&::before": { content: '""', position: "absolute", top: "-20%", right: "-20%", width: "80%", height: "80%", background: "radial-gradient(circle, rgba(59, 130, 246, 0.2) 0%, transparent 70%)", zIndex: -1 } }}>
            <Stack spacing={2.5} sx={{ height: "100%" }}>
              ...
              <Box sx={{ p: 2.5, borderRadius: 3.5, border: "1px solid rgba(255,255,255,0.18)", bgcolor: "rgba(30, 58, 138, 0.3)", backdropFilter: "blur(12px)" }}>
                <Stack direction="row" spacing={1.5} alignItems="center">
                  <Avatar sx={{ bgcolor: "rgba(59, 130, 246, 0.2)", color: "#fff" }}><VerifiedUserRoundedIcon fontSize="small" /></Avatar>
                  <Box>
                ...
                <Box component="form" onSubmit={onSubmit} sx={{ p: { xs: 2, md: 3.5 }, borderRadius: 4.5, border: "1px solid rgba(15, 23, 42, 0.08)", bgcolor: "rgba(255,255,255,0.45)", backdropFilter: "blur(12px)" }}>
                  <Stack spacing={3.2}>
                    <Box>
                      ...
                      <TextField placeholder={copy.usernamePlaceholder} value={username} onChange={(event) => setUsername(event.target.value)} fullWidth InputProps={{ sx: { borderRadius: 2.5, bgcolor: "#fff", "& fieldset": { borderColor: "rgba(15, 23, 42, 0.12)" } }, startAdornment: <EmailIcon sx={{ mr: 1, color: "#1e3a8a", fontSize: 20 }} /> }} />
                    </Box>
                    <Box>
                    ...
                    <Box>
                      ...
                      <TextField
                        ...
                        InputProps={{
                          ...
                          startAdornment: <FingerprintRoundedIcon sx={{ mr: 1, color: "#1e3a8a", fontSize: 20 }} />
                        }}
                      />
                    </Box>
                    {error ? <Alert severity="error" sx={{ borderRadius: 3 }}>{error}</Alert> : null}
                    <Button type="submit" variant="contained" size="large" disabled={loading} sx={{ height: 54, borderRadius: 3.5, fontWeight: 900, fontSize: "1rem", bgcolor: "#1e3a8a", "&:hover": { bgcolor: "#1e3a8a" }, boxShadow: "0 10px 20px -5px rgba(30, 58, 138, 0.3)" }}>
                      {loading ? <CircularProgress size={24} sx={{ color: "inherit" }} /> : copy.submit}
                    </Button>
                  </Stack>
                </Box>
                <Typography variant="body2" sx={{ textAlign: "center", color: "text.secondary" }}>{copy.helperNote}</Typography>
              </Stack>
            </CardContent>
          </Grid>
        </Grid>
