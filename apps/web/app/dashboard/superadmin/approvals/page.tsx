...
export default function SuperadminApprovalsPage() {
  ...
  const rejectedSocieties = useMemo(
    () => monitoringOverview?.societies.filter((s) => s.status === "SUSPENDED") ?? [],
    [monitoringOverview]
  );
  ...
  const customAccessibleModules = useMemo(
    () => [
      ...
      },
      {
        ...
        items: [
          { label: "Approvals & Requests", href: "/dashboard/superadmin/approvals", slug: "approvals", active: true, badge: pendingSocieties?.length > 0 ? String(pendingSocieties.length) : undefined, icon: <VerifiedUserRoundedIcon /> },
      ...
      <Box sx={{ pb: 8 }}>
        <Box sx={{ mb: 6 }}>
          ...
          <Typography variant="subtitle1" sx={{ color: "text.secondary", mt: 1, maxWidth: 800 }}>
            Review new society applications, verify background checks, and activate valid platform access requests.
          </Typography>
        </Box>
        ...
        <Box sx={{ mb: 4, borderBottom: "1px solid rgba(148,163,184,0.15)" }}>
          <Tabs value={activeTab} onChange={(_, v) => setActiveTab(v)}>
            <Tab 
               label={`Pending Approval (${pendingSocieties.length})`} 
               sx={{ fontWeight: 900, textTransform: "none", fontSize: "1rem" }} 
            />
            <Tab 
               label={`Rejected / Suspended (${rejectedSocieties.length})`} 
               sx={{ fontWeight: 900, textTransform: "none", fontSize: "1rem" }} 
            />
          </Tabs>
        </Box>
        ...
        {displayList.length === 0 ? (
           <Box sx={{ p: 6, textAlign: "center", borderRadius: 4, border: "1px dashed rgba(148,163,184,0.3)", bgcolor: "rgba(248,250,252,0.5)" }}>
              <VerifiedUserRoundedIcon sx={{ fontSize: 64, color: "rgba(148,163,184,0.2)", mb: 2 }} />
              <Typography variant="h6" sx={{ color: "text.secondary", fontWeight: 800 }}>No requests found</Typography>
              <Typography variant="body2" sx={{ color: "text.disabled", mt: 1 }}>{activeTab === 0 ? "You're all caught up! No pending applications." : "No rejected applications in the system."}</Typography>
           </Box>
        ) : (
          <Grid container spacing={3}>
            {displayList.map((society) => (
              <Grid size={{ xs: 12, lg: 6 }} key={society.id}>
                <Box sx={{ p: 4, borderRadius: 5, bgcolor: "#fff", border: activeTab === 0 ? "2px solid #fde68a" : "2px solid #fecaca", boxShadow: "0 4px 20px rgba(15,23,42,0.04)" }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                    <Box>
                      <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
                        <Typography variant="caption" sx={{ color: activeTab === 0 ? "#d97706" : "#dc2626", fontWeight: 900, letterSpacing: 1.5 }}>ID: {society.code}</Typography>
                      ...
                      <Stack direction="row" spacing={4} sx={{ mt: 2 }}>
                        <Box>
                          <Typography variant="caption" sx={{ color: "text.disabled", fontWeight: 800 }}>PROPOSED MEMBERS</Typography>
                          <Typography variant="h6" sx={{ fontWeight: 900 }}>{society.customers}</Typography>
                        </Box>
                        <Box>
                          <Typography variant="caption" sx={{ color: "text.disabled", fontWeight: 800 }}>INITIAL ACCOUNTS</Typography>
                      ...
                      {activeTab === 0 ? (
                        <>
                          <Button
                            ...
                            onClick={() => handleAction(society.id, "APPROVE")}
                            sx={{ py: 1, px: 3, borderRadius: 3, fontWeight: 900, bgcolor: "#10b981", boxShadow: "none", "&:hover": { bgcolor: "#059669" } }}
                          >
                            {processing === society.id ? <CircularProgress size={20} color="inherit" /> : "Approve"}
                          </Button>
                          <Button 
                            variant="outlined" 
                            disabled={processing === society.id}
                            onClick={() => handleAction(society.id, "REJECT")}
                            sx={{ py: 1, px: 3, borderRadius: 3, fontWeight: 800, borderColor: "#fca5a5", color: "#ef4444", "&:hover": { bgcolor: "#fef2f2", borderColor: "#ef4444" } }}
                          >
                            Reject
                          </Button>
                        </>
                        ...
                        <Button
                          ...
                          disabled={processing === society.id}
                          onClick={() => handleAction(society.id, "APPROVE")}
                          sx={{ py: 1, px: 3, borderRadius: 3, fontWeight: 900, bgcolor: "#3b82f6", boxShadow: "none", "&:hover": { bgcolor: "#2563eb" } }}
                        >
                          {processing === society.id ? <CircularProgress size={20} color="inherit" /> : "Re-evaluate"}
                        </Button>
                      )}
                    </Stack>
                  </Stack>
